...
@property (strong, nonatomic) UILabel *MA12Label;
@property (strong, nonatomic) UILabel *MA26Label;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _MA12Label = [self private_createLabel];
        _MA26Label = [self private_createLabel];
        ...
        _MA12Label.textColor = [UIColor ma12Color];
        _MA26Label.textColor = [UIColor ma26Color];
        ...
        [_MA12Label mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(_MA7Label.mas_right);
            make.top.equalTo(_MA7Label.mas_top);
            make.height.equalTo(self.mas_height).multipliedBy(0.5);
            
        }];
        [_MA26Label mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(_MA12Label.mas_right);
            make.top.equalTo(_MA7Label.mas_top);
            make.height.equalTo(self.mas_height).multipliedBy(0.5);
            
        }];
        [_MA30Label mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(_MA26Label.mas_right);
            make.top.equalTo(_MA7Label.mas_top);
            make.height.equalTo(self.mas_height).multipliedBy(0.5);
        }];
        ...
        [_dateDescLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.mas_left);
            make.top.equalTo(_MA7Label.mas_bottom).offset(3);
            make.height.equalTo(self.mas_height).multipliedBy(0.5);
        ...
        _dateDescLabel.hidden = YES;
        
        _openDescLabel.hidden = YES;
        _closeDescLabel.hidden = YES;
        _highDescLabel.hidden = YES;
        _lowDescLabel.hidden = YES;
...
-(void)maProfileWithModel:(Y_KLineModel *)model
{
    ...
    _MA12Label.text = [NSString stringWithFormat:@" 10:%.2f ",model.MA12.floatValue];//MA
    _MA26Label.text = [NSString stringWithFormat:@" 20:%.2f ",model.MA26.floatValue];//MA
    _MA30Label.text = [NSString stringWithFormat:@" 30:%.2f",model.MA30.floatValue];//MA
}
...
-(void)showPriceLabel:(BOOL)b
{
    ...
    _openDescLabel.hidden = !b;
    _closeDescLabel.hidden = !b;
    _highDescLabel.hidden = !b;
    _lowDescLabel.hidden = !b;
    
    _openLabel.hidden = !b;
    _highLabel.hidden = !b;
    _lowLabel.hidden = !b;
    _closeLabel.hidden = !b;
}
