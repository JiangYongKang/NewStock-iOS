//
//  TaoSkillHistoryListViewController.m
//  NewStock
//
...
@interface TaoSkillHistoryListViewController ()<UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) TaoSkillHistoryListAPI *listAPI;
...
@implementation TaoSkillHistoryListViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (TaoSkillHistoryListAPI *)listAPI {
    if (_listAPI == nil) {
        _listAPI = [TaoSkillHistoryListAPI new];
        _listAPI.page = @"1";
        _listAPI.count = @"5";
...
- (UIView *)header {
    ...
    [lb3 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(view);
        make.centerX.equalTo(view).offset(0 * kScale);
    }];
