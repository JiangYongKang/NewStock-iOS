//
//  TaoLimitAnalysisViewController.m
//  NewStock
//
...
static NSString *cellID = @"TaoLimitAnalysisViewControllerCell";


@interface TaoLimitAnalysisViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation TaoLimitAnalysisViewController
...
- (void)share {
    NSString *url = [NSString stringWithFormat:@"%@%@",API_URL,H5_DB0001];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TaoLimitAnalysisTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    
    TaoCXZYStockModel *model = self.dataArray[indexPath.row];
    
    [cell setN:model.n s:model.s t:model.t zx:model.zx zdf:model.zdf r:model.r tm:model.llt openCount:model.oln];
    return cell;
}
...
- (UITableView *)tableView {
    if (_tableView == nil) {
        ...
        [_tableView registerClass:[TaoLimitAnalysisTableViewCell class] forCellReuseIdentifier:cellID];
...
- (UIView *)header {
    ...
    [lb2 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(view.mas_centerX).offset(-22 * kScale);
        make.centerY.equalTo(view);
    }];
