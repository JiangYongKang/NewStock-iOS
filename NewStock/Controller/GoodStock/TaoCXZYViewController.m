...
static NSString *cellID = @"TaoCXZYViewControllerCell";

@interface TaoCXZYViewController () <UITableViewDelegate,UITableViewDataSource>
...
@implementation TaoCXZYViewController
...
- (void)share {
    NSString *url = [NSString stringWithFormat:@"%@%@",API_URL,H5_DB0003];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TaoCXZYTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    
    TaoCXZYStockModel *model = self.dataArray[indexPath.row];

    [cell setN:model.n s:model.s t:model.t zx:model.zx zdf:model.zdf continueCount:model.cln openCount:model.odn];
    return cell;
}
...
- (UITableView *)tableView {
    if (_tableView == nil) {
        ...
        [_tableView registerClass:[TaoCXZYTableViewCell class] forCellReuseIdentifier:cellID];
