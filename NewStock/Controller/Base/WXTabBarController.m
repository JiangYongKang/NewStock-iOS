...
- (void)viewDidLoad {
    ...
    _backingSelectedIndex = 0;
    
    [self.view insertSubview:self.scrollView belowSubview:self.tabBar];
    self.tabBar.barTintColor = [SystemUtil hexStringToColor:@"#ffffff"];
    self.tabBar.tintColor = [SystemUtil hexStringToColor:@"#0071e1"];
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!self.initialized) {
        ...
        self.tab = [[TabbarView alloc] initWithFrame:CGRectMake(0, -1, MAIN_SCREEN_WIDTH, 50)];
        _tab.backgroundColor = [UIColor whiteColor];
        __weak typeof(self)weakSelf = self;
        _tab.selectedBlcok = ^(NSInteger index) {
            [weakSelf setSelectedIndex:index];
            [weakSelf.delegate tabBarController:weakSelf didSelectViewController:weakSelf.backingViewControllers[index]];
        };
        [self.tabBar addSubview:_tab];
        [self.tabBar bringSubviewToFront:_tab];
    }
}
