...
- (void)layoutSubviews {
    ...
    if (self.isExpanded) {
        self.textLabel.textColor = self.expandedColor;
        self.accessoryView.transform = CGAffineTransformMakeRotation(0);
    } else {
        self.textLabel.textColor = self.planColor;
    }
}

static UIImage *_image = nil;
static UIImage *_selImage = nil;
- (UIView *)expandableView {
    if (!_image) {
        _image = [UIImage imageNamed:@"open_icon"];//@"expandableImage.png"
        _selImage = [UIImage imageNamed:@"close_icon"];
    }
    ...
    [button setBackgroundImage:_selImage forState:UIControlStateSelected];
