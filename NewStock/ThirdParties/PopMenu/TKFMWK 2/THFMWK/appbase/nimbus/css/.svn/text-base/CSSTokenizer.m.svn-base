...
char* subChars(char* ch,int pos,int length)
{
    char* pch = ch;
...
#line 35 "css.grammar"
{
    NSString *content = [TKDataHelper charsToString:csstext];
    NSInteger index = [TKStringHelper string:content indexOf:@":"];
    if(index > 0)
    {
        NSString *temp = [TKStringHelper subStringWith:content fromIndex:index toIndex:content.length];
        if([TKStringHelper string:@":selected;:highlighted;:disabled;" indexOf:[NSString stringWithFormat:@"%@;",temp]] > -1)
        {
            cssConsume(csstext, CSSIDENT);
        }
        else
        {
            int i = [NSNumber numberWithInteger:index].intValue;
            int length = [NSNumber numberWithInteger:content.length].intValue;
            char *value1 = subChars(csstext, 0, i);
            char *value2 = subChars(csstext, i, 1);
            char *value3 = subChars(csstext, i + 1, length - i - 1);
            cssConsume(value1, CSSIDENT);
            cssConsume(value2, CSSUNKNOWN);
            cssConsume(value3, CSSIDENT);
        }
    }
    else
    {
