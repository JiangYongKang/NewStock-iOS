...
//    self.bton = [UIButton buttonWithType:(UIButtonTypeCustom)];
//    _bton.frame = CGRectMakeEx(0, 350, 320, 60);
//    [_bton setTitle:@"跳过" forState:(UIControlStateNormal)];
//    _bton.titleLabel.font = [UIFont systemFontOfSize:13];
//    [_bton setBackgroundImage:[UIImage imageNamed:@"botton_bg"] forState:(UIControlStateNormal)];
//    [_bton setBackgroundImage:[UIImage imageNamed:@"botton_bg_press"] forState:(UIControlStateHighlighted)];
//    [_bton addTarget:self action:@selector(skipIntroduction) forControlEvents:(UIControlEventTouchUpInside)];
//    [self addSubview:_bton];
...
- (UIView *)viewForPage:(EAIntroPage *)page atXIndex:(CGFloat *)xIndex {
    ...
    *xIndex += self.scrollView.frame.size.width;
    ...
    if(page.titleIconView) {
        ...
        if (self.frame.size.width == 320 && self.frame.size.height == 480) {
            titleImageView.frame = CGRectMake(0, 0, 320, 485);
        }else {
            titleImageView.frame = CGRectMakeEx(0, 0, 320, 568);
        }
        NSLog(@"titleImage=======%@",NSStringFromCGRect(titleImageView.frame));
...
- (void)buildFooterView {
    ...
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(0, self.pageControl.frame.origin.y-80, self.scrollView.frame.size.width, 80)];
